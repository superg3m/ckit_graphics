...
CKIT_Vector3 project_to_screen(CKIT_Vector3 point, int width, int height, int y_offset) {
    CKIT_Vector3 screen_pos;
    screen_pos.x = (int)((point.x + 1.0) * width / 2.0);
    screen_pos.y = (int)((point.y + 1.0) * height / 2.0) + y_offset;
	screen_pos.z = 0;
    return screen_pos;
}
